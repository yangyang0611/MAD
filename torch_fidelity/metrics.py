...
def calculate_metrics_one_feature_extractor(**kwargs):
    ...
    input1, input2, input3, input4 = (
        get_kwarg("input1", kwargs),
        get_kwarg("input2", kwargs),
        get_kwarg("input3", kwargs),
        get_kwarg("input4", kwargs),
    )
    ...
    if have_other_than_ppl:
        ...
        if input2 is not None:
            vprint(verbose, "Extracting features from input2")
            featuresdict_2 = extract_featuresdict_from_input_id_cached(2, feat_extractor, **kwargs)

        if input3 is not None:
            vprint(verbose, "Extracting features from input3")
            featuresdict_3 = extract_featuresdict_from_input_id_cached(3, feat_extractor, **kwargs)
            for key, val in featuresdict_3.items():
                assert featuresdict_1[key].shape == val.shape
                featuresdict_1[key] -= val

        if input4 is not None:
            vprint(verbose, "Extracting features from input4")
            featuresdict_4 = extract_featuresdict_from_input_id_cached(4, feat_extractor, **kwargs)
            for key, val in featuresdict_4.items():
                assert featuresdict_2[key].shape == val.shape
                featuresdict_2[key] -= val
        ...
        if have_prc:
            metric_prc = prc_featuresdict_to_metric(
                featuresdict_1, featuresdict_2, feature_layer_prc, **kwargs
            )
