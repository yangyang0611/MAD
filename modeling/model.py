...
class DiffuserTextModel(torch.nn.Module):
    ...
    def state_dict(self, destination=None, prefix="", keep_vars=False):
        return self.unet.state_dict(destination, prefix, keep_vars)

    def load_state_dict(self, state_dict, strict=True):
        self.unet.load_state_dict(state_dict, strict)

    def train(self, mode=True):
        self.unet.train(mode)

    def forward(self, x, time, text, class_labels=None):
        with torch.no_grad():
            prompt_embeds = self.text_encoder(text)[0]
        return self.unet(
            x,
            time,
...
def build_model(cfg):
    return DhariwalUNet(
        img_resolution=cfg.TRAIN.IMAGE_SIZE,
        in_channels=cfg.MODEL.IN_CHANNELS,
        out_channels=cfg.MODEL.OUT_CHANNELS,
